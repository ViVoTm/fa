...
local function block_word(receiver, wordblock)
    ...
    send_large_msg(receiver, wordblock..'غیر مجاز شد❕')
end
...
local function unblock_word(receiver, wordblock)
    ...
    if data[tostring(chat_id)]['blocked_words'][wordblock] then
        ...
        save_data(_config.moderation.data, data)
        send_large_msg(receiver, wordblock..' مجاز شد❕')
    else
        --send_large_msg(receiver, 'کلمه "'..wordblock..'" در لیست غیر مجاز نیست❕')
    end
end
...
local function run(msg, matches)
  ...
  if matches[1] == "غیر مجاز" then 
    if not is_momod(msg) then
      return "فقط برای مدیران در دسترس است❕"
    end
  ...
  if matches[1] == "لیست کلمات" then
    local text = 'لیست کلمات غیر مجاز برای ['..msg.to.title..']:\n\n'
    local i = 1
...
--کپی بدون ذکر نام ادیتور شرعا حرام است
--#code_X
